...
class BaseFirebaseService {
    ...
    func uploadImage(image: UIImage?,
                     ...
                     success: @escaping (String) -> Void,
                     failure: @escaping (_ message: String) -> Void) {
        guard let image = image, let fileName = fileName else {
            failure("image nil")
            return
        }
        let file = [String(Date().timeIntervalSince1970), fileName].joined()
        let storageRef = storage.child("images").child(file)
...
extension BaseFirebaseService {
    ...
    func rxUploadImage(image: UIImage, fileName: String?) -> Observable<String> {
        Observable<String>.create { observable -> Disposable in
